...
import { CortexStep, FunctionlessLLM, OpenAILanguageProgramProcessor, brainstorm, externalDialog, internalMonologue, queryMemory } from "../../src/next";


describe.only("Evil Characters", () => {
  ...
  fairyTales.forEach(({ name, description, initialMessage }) => {
    it(`it does a long ${name} monologue`, async () => {
      const memory = [
        {
      ...
      const summary = await action.next(queryMemory(`What is ${step.entityName} thinking about right now?`))
      console.log("summary", summary.toString())
    })
  })
})