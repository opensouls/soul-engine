...
import { MentalProcess, mentalQuery, useProcessManager, useActions } from "@opensouls/engine";
import { brainstorm, decision, externalDialog } from "./lib/cognitiveFunctions";
...
const provokesSpeaker: MentalProcess = async ({ workingMemory }) => {
  ...
  [workingMemory, response] = externalDialog(workingMemory, "Try to provide the speaker")
  speak(response)

  // decide if it's time to move on
  const [, didProvokeTheUser] = mentalQuery(workingMemory, "Has samantha successfully provoked the speaker?")
